...
**Automate.** Workflows that run your business proactively. 25 workflow patterns (sequence, planner-executor, checkpoint, fork/join, autonomous loop, multi-agent swarm, and more). Heartbeat scheduling for proactive agent execution. Natural-language scheduling — type "run my marketing report every Monday at 9am" and it works.
...
- **Browser layer** — React 19 with 34 operator surfaces, real-time SSE streaming
- **Server layer** — Next.js 16 with 20 service modules, 114 API endpoints
- **External layer** — Local SQLite database (WAL mode, 30+ tables), no cloud dependency

Technology stack: TypeScript (85% of codebase), Tailwind CSS v4, shadcn/ui, Drizzle ORM. Open source under Apache 2.0.

### What's Shipped Today

| Capability | Status |
|-----------|--------|
| 110 features across 34 operator surfaces | Shipped |
| 52+ specialist agent profiles (incl. 6 business-function profiles) | Shipped |
| 25 workflow patterns with blueprint catalog | Shipped |
