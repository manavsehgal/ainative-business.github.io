...
**Automate.** Workflows that run your business proactively. 31 workflow patterns (sequence, planner-executor, checkpoint, fork/join, autonomous loop, multi-agent swarm, and more). Heartbeat scheduling for proactive agent execution. Natural-language scheduling — type "run my marketing report every Monday at 9am" and it works.
...
- **Browser layer** — React 19 with 39 operator surfaces, real-time SSE streaming
- **Server layer** — Next.js 16 with 33 service modules, 149 API endpoints
- **External layer** — Local SQLite database (WAL mode, 45+ tables), no cloud dependency

Technology stack: TypeScript (87% of codebase), Tailwind CSS v4, shadcn/ui, Drizzle ORM. Open source under Apache 2.0.

### What's Shipped Today

| Capability | Status |
|-----------|--------|
| 149 features across 39 operator surfaces | Shipped |
| 52+ specialist agent profiles (incl. 6 business-function profiles) | Shipped |
| 31 workflow patterns with blueprint catalog | Shipped |
